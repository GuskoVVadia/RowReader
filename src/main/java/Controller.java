/**
 * Объект этого класс выполняет функции Контроллера из схемы MVC.
 * В данном случае класс реализует два интерфейса:
 * Интерфейс слушателей размеров окна и интрерфейс формирования окна при возникновении ошибки.
 * Реализованы методы получения пути к пользовательскому файлу и получения кодировки файла.
 * Именно в Контроллере создаются экземпляры Model и View, а также запускаются слушатели клавиатуры,
 * слушатели размеров окна (с небольшой задержкой), запускается непосредственно окно приложения.
 */
